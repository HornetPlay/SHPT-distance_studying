...
int verify_knownhost(ssh_session session) {
    ...
    case SSH_SERVER_NOT_KNOWN:
        ...
        if (strncmp(buf, "yes", 3) != 0)  // изменил strncasecmp на strncmp !!!
        {
            free(hash);
            return -1;
        }
...
int main(int argc, char** argv) {
    ...
    ssh_options_set(session, SSH_OPTIONS_HOST, "localhost"); // было - host -  указал хост напрямую
    //    ssh_options_set(session, SSH_OPTIONS_LOG_VERBOSITY, &verbosity);
    ssh_options_set(session, SSH_OPTIONS_PORT, &port); // порт
    ssh_options_set(session, SSH_OPTIONS_USER, "user"); // юзер

    printf("Connecting to host %s and port %d\n", "localhost" , port); // указал хост напрямую
    connection = ssh_connect(session);

    if (connection != SSH_OK) {
        printf("Error connecting to %s: %s\n", "localhost", ssh_get_error(session));
        exit(-1);
    }
    else {
        printf("Connected.\n");
 ...
 /*   char* password;*/
    ...
    char* password = new char[pswd.length() + 1];
    std::strcpy(password, pswd.c_str());
    /*-------------*/
